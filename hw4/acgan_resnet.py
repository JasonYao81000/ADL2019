...
class BasicBlock_Generator(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, upsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None):
        super(BasicBlock_Generator, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock only supports groups=1 and base_width=64')
        ...
        self.conv1_upsample = nn.Upsample(scale_factor=stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes)
...
class Bottleneck_Generator(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, upsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None):
        super(Bottleneck_Generator, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2_upsample and self.upsample layers upsample the input when stride != 1
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, groups, dilation)
        self.conv2_upsample = nn.Upsample(scale_factor=stride)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
...
def _resnet_Generator(arch, block, layers, **kwargs):
    model = ResNet_Generator(block, layers, **kwargs)
    return model
...
class Generator(nn.Module):
    def __init__(self, z_dim, datasets):
        ...
        self.init_size = IMAGE_SIZE // 8  # Initial size before upsampling
        self.l1 = nn.Sequential(nn.Linear(z_dim + datasets.labels.shape[1], 64 * self.init_size ** 2))

        self.resnet = _resnet_Generator('resnet18', BasicBlock_Generator, [2, 2, 2, 2])

        self.conv_blocks = nn.Sequential(
            self.resnet,
            nn.Conv2d(512, IMAGE_CHANNELS, 3, stride=1, padding=1),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    def __init__(self, datasets):
        ...
        self.resnet = _resnet('resnet18', BasicBlock, [2, 2, 2, 2])
        flatten_size = 512 * BasicBlock.expansion
        ...
        self.aux_layer_glasses = nn.Sequential(nn.Linear(flatten_size, len(datasets.attr_glasses)), nn.Softmax(dim=-1))
