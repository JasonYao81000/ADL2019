...
class Discriminator(nn.Module):
    def __init__(self, datasets):
        ...
        # Output layers
        self.adv_layer = nn.Sequential(nn.utils.spectral_norm(nn.Linear(flatten_size, 1)))
        self.aux_layer_hair = nn.Sequential(nn.utils.spectral_norm(nn.Linear(flatten_size, len(datasets.attr_hair))), nn.Softmax(dim=-1))
        self.aux_layer_eye = nn.Sequential(nn.utils.spectral_norm(nn.Linear(flatten_size, len(datasets.attr_eye))), nn.Softmax(dim=-1))
        self.aux_layer_face = nn.Sequential(nn.utils.spectral_norm(nn.Linear(flatten_size, len(datasets.attr_face))), nn.Softmax(dim=-1))
        self.aux_layer_glasses = nn.Sequential(nn.utils.spectral_norm(nn.Linear(flatten_size, len(datasets.attr_glasses))), nn.Softmax(dim=-1))
