...
class Discriminator(nn.Module):
    def __init__(self, datasets):
        ...
        self.aux_layer_hair = nn.Sequential(nn.Linear(flatten_size, 6), nn.Softmax(dim=-1))
        self.aux_layer_eye = nn.Sequential(nn.Linear(flatten_size, 4), nn.Softmax(dim=-1))
        self.aux_layer_face = nn.Sequential(nn.Linear(flatten_size, 3), nn.Softmax(dim=-1))
        self.aux_layer_glasses = nn.Sequential(nn.Linear(flatten_size, 2), nn.Softmax(dim=-1))
