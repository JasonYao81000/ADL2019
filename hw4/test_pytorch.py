...
def main():
    ...
    print(opt.test)
    if opt.test:
        generator.load_state_dict(torch.load(os.path.join(opt.ckpt_dir, 'generator_gd_120.cpt')))
        generator.eval()
        discriminator.load_state_dict(torch.load(os.path.join(opt.ckpt_dir, 'discriminator_gd_120.cpt')))
        discriminator.eval()
        if opt.mode == 'human':
...
#            print()
#            record_acc = a
        if d<0.21 and g<0.91:
            torch.save(generator.state_dict(), opt.ckpt_dir + 'generator_gd_%d.cpt' % save_m)
            torch.save(discriminator.state_dict(), opt.ckpt_dir + 'discriminator_gd_%d.cpt' % save_m)
            print("save_gdmodel")
            print()
            save_m += 1
#            record_gd = g*d
