...
class EmbBiGruBattMaxFocalNet(torch.nn.Module):
    """
    Args:
    """

    def __init__(self, embedding, device, dropout_rate=0.2, similarity='MLP'):
        super(EmbBiGruBattMaxFocalNet, self).__init__()
        ...
        self.num_layers = 1
        self.rnn = torch.nn.GRU(embedding.size(1), self.hidden_size, self.num_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, context, context_lens, options, option_lens):
        ...
        options = self.dropout(options)

        context_length = context.size(1)

        # Set initial hidden and cell states 
        h_0 = torch.zeros(self.num_layers * 2, context.size(0), self.hidden_size).to(self.device)
        ...
        for i, option in enumerate(options.transpose(1, 0)):
            ...
            # Set initial hidden and cell states 
            h_1 = torch.zeros(self.num_layers * 2, context.size(0), self.hidden_size).to(self.device)
            ...
            # Set initial hidden and cell states 
            h_1 = torch.zeros(self.num_layers * 2, option.size(0), self.hidden_size).to(self.device)
