...
class DeepBiGruBattMaxFocalNet(torch.nn.Module):
    """
    Args:
    """
    ...
    def __init__(self, dim_embeddings, dropout_rate=0.2, 
                 similarity='MLP'):
        super(DeepBiGruBattMaxFocalNet, self).__init__()
        self.hidden_size = 128
        self.num_layers = 2
        self.rnn = torch.nn.GRU(dim_embeddings, self.hidden_size, self.num_layers, batch_first=True, dropout=dropout_rate, bidirectional=True)
        ...
        self.attention_rnn = torch.nn.GRU((self.hidden_size * 2) * 4, self.hidden_size, self.num_layers, batch_first=True, dropout=dropout_rate, bidirectional=True)
