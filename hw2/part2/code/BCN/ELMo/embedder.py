...
class Embedder:
    ...
    def __init__(self, n_ctx_embs, ctx_emb_dim, predict):
        """
        ...
        if self.predict==1:
            if self.n_ctx_embs == 4 and self.ctx_emb_dim == 1024:
                self.stacked_embeddings = StackedEmbeddings([
                    BertEmbeddings('bert-large-cased'),     # 4096
                ])
        elif self.predict==2:
            if self.n_ctx_embs == 7 and self.ctx_emb_dim == 2048:
                self.stacked_embeddings = StackedEmbeddings([
                    ELMoEmbeddings('original'),             # 3072
                    BertEmbeddings('bert-large-cased'),     # 4096
                ])
        elif self.predict==3 or self.predict==4:
            if self.n_ctx_embs == 7 and self.ctx_emb_dim == 2048:
                self.stacked_embeddings = StackedEmbeddings([
                    ELMoEmbeddings('original'),             # 3072
        ...
        elif self.predict==6 or self.predict==7:     
            if self.n_ctx_embs == 3 and self.ctx_emb_dim == 1024:
                self.stacked_embeddings = StackedEmbeddings([
                    ELMoEmbeddings('original'),             # 3072
                ])
